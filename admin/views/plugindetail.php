...
.plugpress-thumbnail-pagination{text-align: center;}
.plugpress-thumbnail-pagination a{display: inline-block; height: 15px; margin: 0 5px 0 0; width: 15px;}
...
<?php
	if ( !empty($plugpressView->plugin->testedupto) && version_compare( substr($GLOBALS['wp_version'], 0, strlen($plugpressView->plugin->testedupto)), $plugpressView->plugin->testedupto, '>') ) {
	...
	else if ( !empty($plugpressView->plugin->wordpressrequired) && version_compare( substr($GLOBALS['wp_version'], 0, strlen($plugpressView->plugin->wordpressrequired)), $plugpressView->plugin->wordpressrequired, '<') ) {
...
<div id="plugpress-content">
	...
	<div id="plugpress-content-right">
		<div class="plugpress-buynow-box">
			...
			<?php if ($plugpressView->plugin->price == 0) : ?>
...
<form id="plugpress-form-buy" action="" method="POST" target="_blank">
	<input type="hidden" name="username" value="<?php esc_attr_e($plugpressView->username) ?>" />
	<input type="hidden" name="slug" value="<?php esc_attr_e($plugpressView->plugin->slug) ?>" />
</form>
...
function plugpress_buyit() {
	...
	f.get(0).setAttribute('action', '<?php echo PLUGPRESS_WEBSITE; ?>buy/plugin');
	f.submit();
